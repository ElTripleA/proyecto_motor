...
float variables[6];	//Gracias a variables en la posición 6 guaradamos los datos en el tiempo anterior y además se guardarán los distintos datos separados a lo largo del siguiente ciclo de extracción
...
{
    QString temp;
    temp = "Velocidad "+QString::number(variables[1],'f', 2)+"\nFrecuencia "+QString::number(variables[2],'f', 2)+"\nVoltaje "+QString::number(variables[3],'f', 2)+"\nTempertura "+QString::number(variables[4],'f', 2);
    ...
    struct tm tiempo_lugar;//Se definio "tm" como parte de la estructura dentro del time.h
    char tiempo[50];//Se define un arreglo en donde se guardará el tiempo
    ...
    {
        proceso = fork();//Se realiza el uso del fork para utilizar distintas funcionalidades
        switch (proceso)
        {
            case 0: //Se abre el proceso hijo, si el PID creado es 0
            ...
            default: //Proceso padre, si el PID creado es distinto a 0
                    pid_hijo = wait(NULL); //Espera a que el proceso hijo haya terminado para proceder con el padre

                    FILE *apertura_txt;//Se realiza el proceso de apertura del archivo creado por el hijo
                    ...
                    rewind(apertura_txt);
                    fread(informacion_txt, 1, size, apertura_txt);//Se guarda la cadena obtenida del txt en informacion_txt
                    ...
                    for(contador=0;contador<size;contador++)//Se realiza ciclo de identificación de todo el archivo hasta un tope de tamaño size
                    {
                        if(informacion_txt[contador]!=' ' && informacion_txt[contador]!='\0')//Si en la información se encuentra algo distinto al ' ' y al '\0'
                        {
                            datos_del_txt[datos_cont]=informacion_txt[contador];//Agregue al vector "datos_del_txt" la información encontrada entre coincidencias según el condicional anterior
                            datos_cont++;//Como hubo una coincidencia aumente el datos_cont para registrar en un nuevo espacio una futura inicidencia
                        }
                        if(informacion_txt[contador]==' ' || informacion_txt[contador]=='\0')//Si en la información hay un ' ' o un '\0'
                        {
                            variables[variables_cont]=atof(datos_del_txt);//Convierta lo almacenado dentro de la incidencia, a un float, en la posición de "variables"
                            datos_cont=0;//Se reinicia el contador de los datos
                            variables_cont++;//Como hubo una coincidencia aumente el datos_cont para registrar en un nuevo espacio una futura inicidencia
                            datos_del_txt[0]='\0';
                        }
                    }
                    ...
                    {
                        ui->Velocidad->setText("Vel. "+QString::number(variables[1],'f', 2)); // pintar de rojo con una precisión de 2.
                        ui->Velocidad->setStyleSheet("background-color:rgb(239, 41, 41)");
                        Ventana::on_checkVelo_stateChanged(1);
                    }
                    else
                    {
                        ui->Velocidad->setText("Vel. "+QString::number(variables[1],'f', 2)); // pintar de verde.
                    ...
                    {
                        ui->Frecuencia->setText("Frec. "+QString::number(variables[2],'f', 2)); // pintar de rojo.
                        ui->Frecuencia->setStyleSheet("background-color:rgb(239, 41, 41)");
                        Ventana::on_checkFrecuen_stateChanged(1);
                    }
                    else
                    {
                        ui->Frecuencia->setText("Frec. "+QString::number(variables[2],'f', 2)); // pintar de verde.
                    ...
                    {
                        ui->Voltaje->setText("Volt. "+QString::number(variables[3],'f', 2)); // pintar de rojo.
                        ui->Voltaje->setStyleSheet("background-color:rgb(239, 41, 41)");
                        Ventana::on_checkVoltaje_stateChanged(1);
                    }
                    else
                    {
                        ui->Voltaje->setText("Volt. "+QString::number(variables[3],'f', 2)); // pintar de verde.
                    ...
                    {
                        ui->Temperatura->setText("Temp. "+QString::number(variables[4],'f', 2)); // pintar de rojo.
                        ui->Temperatura->setStyleSheet("background-color:rgb(239, 41, 41)");
                        Ventana::on_checkTemp_stateChanged(1);
                    }
                    else
                    {
                        ui->Temperatura->setText("Temp. "+QString::number(variables[4],'f', 2)); // pintar de verde.
                    ...
                    tiempo_op = variables[5]/1000;//Se divide entre mil el tiempo captado en la varible 5, dado que el formato final no era congruente
                    time_t rawtime=tiempo_op;  //Se le asigna lo contenido dentro de tiempo op a "RAWTIME" con el fin de generar la asosciación al tiempo real
                    tiempo_lugar=*localtime(&rawtime);//Asosciamos mediante la estructura el tiempo local obtenido en la anterior variable
                    strftime(tiempo, sizeof(tiempo),"%a %Y-%m-%d %H:%M:%S %Z",&tiempo_lugar); //Da formato gracias al timestamp
                    ui->TiempoOpera->setText(tiempo);
                    ui->Historial->setText(temp+"\n\n"+"Velocidad "+QString::number(variables[1],'f', 2)+"\nFrecuencia "+QString::number(variables[2],'f', 2)+"\nVoltaje "+QString::number(variables[3],'f', 2)+"\nTempertura "+QString::number(variables[4],'f', 2)); //Formato para el cuadro de texto de historial
